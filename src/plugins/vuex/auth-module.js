...
export default {
  ...
  getters: {
    ...
    isLoggedIn: (state) =>
      state.tokens.access_token !== null && state.tokens.refresh_token !== null,
    redirectUrl: (state) => 'http://localhost:' + state.port,
    oauth: (state, getters) => {
      let client = new google.auth.OAuth2(state.ytId, state.ytSecret, getters.redirectUrl);
  ...
  actions: {
    ...
    async processAuth({ commit, getters }) {
      let result = await service.channels.list({
        auth: getters.oauth,
        part: 'snippet',
        mine: true,
      });
