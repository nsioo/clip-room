<template>
  <perfect-scrollbar class="graph">
    ...
    <v-card
      :color="`secondary darken-${chain.inputs.includes('a') ? '2' : '0'}`"
      class="node"
      v-for="chain in complexFilter"
    >
    ...
    <v-card color="success" class="node output">
      <v-card-text>
        <v-card-title class="node-input"
          >Map <b class="mr-1 ml-1">out</b> to output file</v-card-title
        >
      </v-card-text>
    </v-card>
  </perfect-scrollbar>
