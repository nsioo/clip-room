<template>
  <v-sheet class="history-container">
    ...
    <perfect-scrollbar class="history">
      <h4 class="text-center mt-3">历史操作</h4>
      <v-list color="transparent" dense rounded>
        <v-list-item
          ...
          v-for="(command, i) in visualCommands"
          :key="i"
...
export default {
  ...
  methods: {
    revertHistory(index) {
      ...
      if (count < 0) for (let i = 0; i > count; i--) this.undo();
      else if (count > 0) for (let i = 0; i < count; i++) this.redo();
    },
